...
#include <SoftwareSerial.h>   // Para la comunicación  (aunque no se use directamente en esta versión simplificada)
...
public:
  ...
  //Junta los valores de todos los sensores.  -- Simplificado para un solo sensor.
...
truncar:
  ...
  Mensaje construir() {
    ...
    char resultado[125];                                      //Variable para guardar el mensaje.
...
public:
  ...
  int getLux() {
    ...
    if (lectura < 0) {
      return -999;  // Representa NaN como -999 ya que en Arduino no podemos retornar strings
    }
    return int(lectura);
  }
  ...
  void componenteNotificar() override {
    ...
    char valor[15];
    itoa(valorLux, valor, 10);  //Convierte el valor a char*.
...
/*class RelojComponent : public Sujeto, public IInicializador {
    ...
    public:
        ...
        void componenteNotificar() override {
            ...
            char fechaHora[50];
            snprintf(fechaHora, sizeof(fechaHora), "%04d%02d%02d%02d%02d%02d", now.year(), now.month(), now.day(), now.hour(), now.minute(), now.second());
...
//-----------------------//
BH1750Component sensorLuz;  // Instancia del sensor.
//RTC_DS3231 rtc; // Instancia del reloj.
//RelojComponent reloj(rtc); // Instancia del reloj.
...
Task tareaMoverPanel(30000, TASK_FOREVER, []() {
